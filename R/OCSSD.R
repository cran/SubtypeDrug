##' OCSSD
##'
##' @title Optimizing cancer subtype specific drugs (OCSSD)
...
##' @param input.drug.data A list data of drug regulation. The drug subpathway association data we constructed is stored in package `SubtypeDrugData` and can be downloaded via connection \url{https://github.com/hanjunwei-lab/SubtypeDrugData}.
...
##' \donttest{Subtype_drugs<-OCSSD(Geneexp,Subtype,"Control",SpwSymbolList,input.drug.data=DrugSpwData)}
##'
...
OCSSD<-function(expr,input.cls="",control.label="",subpathway.list,
  ...
  drug_target_data<-getDrugSpw(input.drug.data,spw_matrix_rnames,drug.p.val.threshold,drug.spw.min.sz,drug.spw.max.sz)
  ...
  Parameters<-list(control.label=control.label,spw.min.sz=spw.min.sz,spw.max.sz=spw.max.sz,spw.score.method=spw.score.method,kcdf=kcdf,
                    drug.p.val.threshold=drug.p.val.threshold,drug.spw.min.sz=drug.spw.min.sz,drug.spw.max.sz=drug.spw.max.sz,weighted.drug.score=weighted.drug.score,nperm=nperm,parallel.sz=parallel.sz)

   if(length(phen)==1){
      fc<-apply(spw_matrix_y, 1, function(x){
        foldc<-mean(x[which(samples.v1!=control.label)])-mean(x[which(samples.v1==control.label)])
        return(foldc)
      })
      ...
      rdmean_matrix<-apply(rdmean_matrix,2,function(x){
        zdz<-max(x)
        zxz<-min(x)
        for(i in 1:length(x)){
         if(x[i]>0){
